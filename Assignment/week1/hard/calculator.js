// /*
//   Implement a class `Calculator` having below methods
//     - initialise a result variable in the constructor and keep updating it after every arithmetic operation
//     - add: takes a number and adds it to the result
//     - subtract: takes a number and subtracts it from the result
//     - multiply: takes a number and multiply it to the result
//     - divide: takes a number and divide it to the result
//     - clear: makes the `result` variable to 0
//     - getResult: returns the value of `result` variable
//     - calculate: takes a string expression which can take multi-arithmetic operations and give its result
//       example input: `10 +   2 *    (   6 - (4 + 1) / 2) + 7`
//       Points to Note: 
//         1. the input can have multiple continuous spaces, you're supposed to avoid them and parse the expression correctly
//         2. the input can have invalid non-numerical characters like `5 + abc`, you're supposed to throw error for such inputs

//   Once you've implemented the logic, test your code by running
// */

class Calculator {
  constructor() {
    this.result = 0; // Initialize result to 0
  }

  add(number) {
    this.result += number; // Update result with addition
    return this.result; // Return the updated result
  }

  subtract(number) {
    this.result -= number; // Update result with subtraction
    return this.result; // Return the updated result
  }

  multiply(number) {
    this.result *= number; // Update result with multiplication
    return this.result; // Return the updated result
  }

  divide(number) {
    if (number === 0) throw new Error(); // Handle division by zero
    this.result /= number; // Update result with division
    return this.result; // Return the updated result
  }

  clear() {
    this.result = 0; // Reset result to 0
  }

  getResult() {
    return this.result; // Return the current result
  }

  calculate(expression) {
    // Clean up the input expression
    expression = expression.replace(/\s+/g, ''); // Remove all whitespace

    // Validate the expression for invalid characters
    if (/[^0-9+\-*/().]/.test(expression)) {
      throw new Error('Invalid characters in expression');
    }

    // Check for mismatched parentheses
    if (!this.validateParentheses(expression)) {
      throw new Error('Mismatched parentheses');
    }

    try {
      // Replace division by zero with an error
      if (this.checkDivisionByZero(expression)) {
        throw new Error('Division by zero');
      }

      // Evaluate the expression safely
      const result = Function(`'use strict'; return (${expression})`)();
      if (typeof result === 'number') {
        this.result = result; // Update the result
        return this.result; // Return the updated result
      } else {
        throw new Error('Invalid expression');
      }
    } catch (error) {
      throw new Error('Invalid expression');
    }
  }

  validateParentheses(expression) {
    let balance = 0;
    for (const char of expression) {
      if (char === '(') balance++;
      if (char === ')') balance--;
      if (balance < 0) return false; // More closing parentheses
    }
    return balance === 0; // Ensure all parentheses are closed
  }

  checkDivisionByZero(expression) {
    // Simple regex to detect division by zero patterns
    const regex = /\/\s*0\b/;
    return regex.test(expression);
  }
}


module.exports = Calculator;

